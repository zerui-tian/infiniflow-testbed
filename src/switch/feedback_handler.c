...
uint32_t switch_feedback_handler_run_tick(switch_ctx_t *ctx) {
    static bool g_feedback_rx_port_logged[SWITCH_MAX_EGRESS_PORTS];
    ...
    for (egress_idx = 0; egress_idx < ctx->cfg.nb_egress_ports; egress_idx++) {
        ...
        nb_rx = rte_eth_rx_burst(egress_port, ctx->cfg.egress_rx_queue_id, rx_pkts, burst);
        if (nb_rx == 0U) {
            continue;
        }
        if (!g_feedback_rx_port_logged[egress_idx]) {
            RTE_LOG(INFO, USER1, "switch: feedback RX on egress port %" PRIu16 "\n", egress_port);
            g_feedback_rx_port_logged[egress_idx] = true;
        }
